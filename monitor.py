...
commands_available_txt = (
    "Commands available (use /setcommands in the Botfather chat to set these): \n"
)
...
class Monitor:
    ...
    def __init__(self):
        ...
        async def handler(event):
            filepath = self.plot_over_time(yaxis="availability", ylabel="Available")
            async with client.action(username, "photo") as action:
                await client.send_file(
                    username,
                    filepath,
